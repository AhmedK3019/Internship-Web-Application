...
import './index.css';
import { technicalSkillsQuestions, cognitiveQuestions, behavioralQuestions } from './questions';
    ...
    if (submitted || assessment.status === "Completed") {
        return (
            <div className="internship-card" >
                <button onClick={onBack} className="back-button">← Back To Assessments</button>
                <h2 >Assessment Results</h2>
                <p style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: "20px" }}>Your score: {score}%</p>
                <p style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: "20px" }}>{score >= assessment.passingScore ? 'Passed' : 'Failed'}</p>
                <p style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: "20px" }}>Passing Score: {assessment.passingScore}%</p>
                ...
                <div key={q.id} className="question-container">
                    ...
                    <div>
                        {q.options.map((option, aIndex) => (
                            <div
                                key={aIndex}
                                className={`option ${answers[qIndex] === aIndex ? 'selected' : ''}`}
