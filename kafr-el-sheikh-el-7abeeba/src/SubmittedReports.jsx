...
const downloadReport = (report) => {
  ...
  // Helper functions from generatePDF
  ...
  doc.text("INTERNSHIP REPORT", pageWidth / 2, yPosition, { align: "center" });
  yPosition += 15;

  // Student Information
  addSectionHeader("Student Information");
  addInlineText("Name", report.studentName);
  addInlineText("Email", report.studentEmail || "N/A");
  addInlineText("Major", report.major || report.studentMajor || "N/A");
  yPosition += 5;

  // Internship Details
  addSectionHeader("Internship Details");
  addInlineText("Company", report.company || "N/A");
  addInlineText("Title", report.title);
  addInlineText("Supervisor", report.supervisor || "N/A");
  addInlineText("Location", report.location || "N/A");
  addInlineText("Duration", report.duration || "N/A");
  addInlineText("Period", `${report.startDate || "N/A"} to ${report.endDate || "N/A"}`);
  yPosition += 5;

  // Report Content
  addSectionHeader("Introduction");
  addMultilineText("", report.introduction || report.description || "No introduction provided");

  addSectionHeader("Report Body");
  addMultilineText("", report.body || report.description || "No report body provided");

  // Skills and Courses
  if (report.courses && report.courses.length > 0) {
    addSectionHeader("Relevant Courses");
    addMultilineText("", report.courses.join(", "));
  }
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text("Generated by Student Portal", margin, 285);
  }

  doc.save(`${report.studentName.replace(/\s+/g, "_")}_Report.pdf`);
};
